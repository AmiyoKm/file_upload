...
import { clearAuthCookie, getAccessTokenCookieOptions, setAuthCookie, } from "../utils/cookies.js";
...
const changePassword = async (req, res) => {
    ...
    const updatedUser = await UserModel.findByIdAndUpdate(userId, { password: hashedPassword }, { new: true, runValidators: true });
    if (!updatedUser) {
        return res.status(400).json({ success: false, error: "User not found" });
    }
...
const refresh = async (req, res) => {
    ...
    const { userId } = jwt.verify(refreshToken, REFRESH_JWT_SECRET);
    if (!userId) {
        return res
            .status(401)
